<?php  include('../model/class.expert.php');
    ...
    $getrec = $ModelCall->rawQuery("SELECT * FROM `tbl_billing_new`b, `Wo_Users` u  where b.bill_date='2020-03-25' and b.email is not NULL and b.email= u.user_email group by b.email limit 1300,100");
  ...
  // $toArray= array($value['email']=>$value['first_name']);
   ...
   print_r($toArray);
    ...
    function send_sms_for_cam_bill($value){
      ...
      $message= 'Dear '.$value['first_name'].' '.$value['last_name'].', Your CAM Bill due is Rs. '.$value['payable_amount'].'. Click here to pay https://bit.ly/3dzZHXx & view your CAM Bills using Net Banking or Credit Cards. ZERO Transaction Charges* for Net Banking. You can also see your previous bills history online.';
   ...
   //  $numbers='9560889608,'. $value['phone_number'];
     // set post fields
     $post = [
      'sender' => $SENDER,
      'smstype' => $smstype,
