...
include(dirname(__FILE__) .'/../model/class.expert.php');
    ...
    $getmailtobesent = $ModelCall->rawQuery("SELECT * FROM `batch_mail_cron_file` where mail_start_date =CURRENT_DATE() and category_mail='Custom Mail' and `status`='ongoing' limit 1");
    
   if(count($getmailtobesent)<1) {
   $getmailtobesent = $ModelCall->rawQuery("SELECT * FROM `batch_mail_cron_file` where mail_start_date =CURRENT_DATE() and category_mail='Custom Mail' and `status`='tobestarted' limit 1");
   }
