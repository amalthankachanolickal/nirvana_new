...
<body>
    <form method="post" action="controller/save_tenant_checkout.php" name="google-form" id="google-form" onsubmit="return validateForm()">
        <div class="container" style="max-width: 800px;">
            <div class="container mt-5 mb-5 form-container">
                ...
                <div class="col-md-12 reduce-bottom-margin-5 order-md-1 order-sm-2 order-2 text-center mt-3 mb-0 mb-md-3">
                  <p class="bold head-matter">Checkout Form</p>
                ...
                <div id="personContainer">
                  ...
                  <div class="row">
                    <div class="myselect min-title no-pdd">
                      <div class="sn-field">
            ...
            <div class="row">
              ...
              <div class="col-sm-12 col-12 col-md-6 no-pdd">                
                <p>Staying in Nirvana since : 
                    <span class="col-md-2 min-des inline-block no-pdd">
                      <input type="date" class="" id="stay_since" name="stay_since" value="<?= (count($user_data) > 0) ? $user_data[0]['stay_since'] : '' ?>" placeholder="Staying in Nirvana sincse" required>
                    </span>
                </p>
              </div>
            </div>
            ...
            <p>Lease Period : 
                <span class="col-md-3 inline-block no-pdd">
                  <input type="date" class="" id="lease_start_date" name="lease_start_date" value="<?= (count($user_data) > 0) ? $user_data[0]['lease_start_date'] : '' ?>" placeholder="Start Date" required>
                </span>

                <span class="col-md-3 ml-md-4 mt-md-0 mt-3 mt-sm-3 inline-block no-pdd">
                  <input type="date" class="" id="lease_end_date" name="lease_end_date" value="<?= (count($user_data) > 0) ? $user_data[0]['lease_end_date'] : '' ?>" placeholder="End Date" required>
                </span>
            </p>
            ...
            <div class="row">               
              ...
              <div class="col-sm-12 col-12 col-md-4 no-pdd">
                <p>Out date : 
                    <span class="col-md-3 min-des inline-block no-pdd">
                      <input type="date" class="" id="move_out_date" name="move_out_date" value="<?= (count($user_data) > 0) ? $user_data[0]['move_out_date'] : '' ?>" placeholder="Move out date" required onchange="fillCompDate()">
                    </span>
                </p>                
              </div>
            </div>

              <p>I/We hereby declare as of 
                <span class="col-md-3 mt-1 inline-block no-pdd">
                  <input type="date" id="end_date" name="end_date" value="<?= (count($user_data) > 0) ? $user_data[0]['end_date'] : '' ?>" placeholder="Date" required disabled>
              </span>, I/We are moving out of nirvana country.</p>

    <p>(Acceptance of check out is upon clearance from office)</p>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <script>
      ...
      function fillCompDate() {
        let fourteen = new Date(document.getElementById("move_out_date").value);
        let endDateCont = document.getElementById("end_date");
        let year = fourteen.getFullYear();
        let month = fix(fourteen.getMonth() + 1);
        let date = fix(fourteen.getDate());        
    ...
    <script>
     ...
     <?php 
        ...
        if (count($user_data) > 0) {
          if (isset($user_data[0]['fname_second']) && strlen($user_data[0]['fname_second']) > 0) {
            ?>
            <script>
    ...
    <?php
          ...
          if (isset($user_data[0]['fname_third']) && strlen($user_data[0]['fname_third']) > 0) { ?>
            <script>
