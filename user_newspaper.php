...
$getUserNewsInfo = $ModelCall->get("tbl_newspaper_subscription");
$forthisusernews=array();
$i=1;
foreach($getUserNewsInfo as $news){
    $A[$i]=$news['news_id'];
    $A['quent'][$news['news_id']]=$news['quantity'];
    $A['start_date'][$news['news_id']]=$news['start_date'];
    $A['end_date'][$news['news_id']]=$news['end_date'];
    $i++;
}
...
<head>
    ...
    <title>User Newspaper Subscription </title>
...
<?php }?>
    ...
    <table  class="table table-striped table-hover dt-responsive display" id="dataTable">
      ...
      <thead class="bg-primary">
        <tr>
            <th class="first" scope="col" data-priority="1"><span class="fa fa-newspaper-o" aria-hidden="true">&#160;</span></th>
          <th class="second" scope="col" data-priority="2">Name</th>
          <th class="third" scope="col">Language</th>
                <th class="sixth" scope="col">Annual Cost</th>
                            <th class="twelfth" scope="col">Quantity</th>
                            <th class="twelfth" scope="col">Start Date</th>
                            <th class="twelfth" scope="col">End Date</th>
        ...
        <tr>
         ...
         ?>
          ...
          <!-- <td>
           ...
           <td>
              <input type="hidden" name="<?=$value['id']?>" value="<?=$_REQUEST[$value['id']]?>" ><?=$A['quent'][$value['id']]?>
               </td>
               <td>
              <input type="hidden" name="<?=$value['id']?>" value="<?=$_REQUEST[$value['id']]?>" ><?=$A['start_date'][$value['id']]?>
               </td>
               <td>
              <input type="hidden" name="<?=$value['id']?>" value="<?=$_REQUEST[$value['id']]?>" ><?=$A['end_date'][$value['id']]?>
               </td>
    ...
    <?php } else {?>
     <!-- <div>You have not subscribed for any newspapers yet.</div> -->
      <?php }?>
     <div align="right" style="margin-right: 150px;">
