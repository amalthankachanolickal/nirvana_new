...
<body>
    <form method="post" action="controller/save_isolation_form.php" name="google-form" id="google-form" onsubmit="return validateForm()">
        <div class="container" style="max-width: 800px;">
            <div class="container mt-5 mb-5 form-container">
                ...
                <div class="col-md-12 reduce-bottom-margin-5 order-md-1 order-sm-2 order-2 text-center mt-3 mb-0 mb-md-3">
                  ...
                  <p style="margin-top: -10px;">(to be filled after completion of 14 days home quarantine period)</p>
                </div>
                </div>
                <br>
            ...
            <div class="row">             
              ...
              <div class="phone no-pdd">
                <div class="sn-field">
                <input type="text" oninput="vallength(this, 10)" id="app_phone" maxlength="10" placeholder="Phone Number" value="<?= (count($user_data) > 0) ? $user_data[0]['app_phone'] : '' ?>" name="app_phone" onblur="validateNumber()" required>         
                </div>      
              </div>
              <div class="col-md-3 no-pdd">
                <div class="sn-field">
            ...
            <p>Date of return from outside Gurgaon / Contact with effected person : 
                <span class="col-md-3 inline-block no-pdd">
                  <input type="date" class="" id="ret_date" name="ret_date" value="<?= (count($user_data) > 0) ? $user_data[0]['ret_date'] : '' ?>" placeholder="Date of return / contact" required>
                </span>
            </p>

              <p>I  hereby  declare  that  I/We have completed mandatory home quarantine period  of 14 days on <span class="col-md-3 mt-1 inline-block no-pdd">
                  <input type="date" id="tod_date" name="tod_date" value="<?= (count($user_data) > 0) ? $user_data[0]['tod_date'] : '' ?>" placeholder="Date" onchange="onBlurCheck()" required>
              </span> and  I/Wehave  not  developed  any symptoms  of COVID 19such as fever, cough, sore throat, shortness of breath etc during this period,nor have I/Wecome in contact with any COVID 19 affected person during this period.</p>
      ...
      <div class="checky-sec st2">
        <div class="fgt-sec">
          <label class="check-new-box">
            <input type="checkbox" name="confirmation" class="my-check <?php if (isset($_GET['fid'])) { echo 'hide'; }?>" id="confirmation" style="" required>
          <span class="checkmark"></span> </label>
          <small style="font-size: 14px;width: 100%;line-height: 20px;font-weight: 500; color: #545454;">Yes, I have registered with Govt help line 104, understand and agree to this undertaking and the <a href="terms_conditions.php">Terms of Use</a></small> </div>
        <!--fgt-sec end-->
      </div>
    </div>
    <br> <br>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <script>
      ...
      function validateReturnDate() {
        let today = new Date(document.getElementById('tod_date').value);
      ...
      function setMinDate() {
        let retDate = new Date(document.getElementById('ret_date').value);
        let fillInput = document.getElementById('tod_date');
        let fourteen = new Date(retDate.getTime() + ((1000 * 3600 * 24)*14));
        let year = fourteen.getFullYear();
        let month = fix(fourteen.getMonth() + 1);
        let date = fix(fourteen.getDate());

        fillInput.setAttribute("min", year+'-'+month+'-'+date);
        fillInput.setAttribute("defaultValue", year+'-'+month+'-'+date)
      }
