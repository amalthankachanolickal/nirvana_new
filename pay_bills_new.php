...
if($_SESSION['event_ticket']['billid']!=''){
  $udf2=$_SESSION['event_ticket']['billid'];
}elseif ($posted['udf2']!=''){
  $udf2 = $posted['udf2'];
}else{
  $udf2 ="";
...
$ModelCall->where("applied_to",$_SESSION['event_ticket']['ActionCall']);
  ...
  //  echo  $mode;
   // echo $charge;
    ...
    //  echo "in 3";
      ...
      //  echo "in 5";
        $posted['amount'] = (float)$posted['amount']+(float)(.01)*$posted['amount']+(float)(0.18)*(.01)*$posted['amount'];    
       }
     }
...
margin:20x;!important ">
               <div class="row">
                    ...
                    <form action="<?php echo $action; ?>" method="post" name="payuForm">
                        ...
                        <input type="hidden"  name="surl" value="<?php echo SITE_URL;?>/response_payu_new.php"/>
                        <input type="hidden"  name="furl" value="<?php echo SITE_URL;?>/response_payu_new.php"/>
                        <input type="hidden"  name="service_provider" value="payu_paisa"/>
                        <input type="hidden"  name="udf1" value="<?php echo $udf1;?>"/>
                        <input type="hidden"  name="udf2" value="<?php echo $udf2;?>"/>
                       <input type="hidden"  name="enforce_paymethod" id="enforce_paymethod"  value="CC"/> 
                       ...
                       <div class="row form-group">
                          ...
                          <div class="sn-field ">
                           <label> Contribution Amount  <strong style="color:#FF0000;">*</strong></label>
                           <input name="amount" id="amount" value="<?php echo (empty($posted['amount'])) ? $amount : $posted['amount'] ?>" class="form-control"  required readonly/>
                           <!-- <i class="la la-lock"></i>--> </div>
                        </div>
                        <div class="col-lg-12 no-pdd">
                          <div class="sn-field">
                          <label> Select Payment Mode - Additional charges as listed <strong style="color:#FF0000;">*</strong></label>
                          <select id="mode" name="mode" onchange="selectPaymentMode()" class="form-control" > 
                         <?php foreach($PayDetails as $charge) {?>
                            <?php if($charge['mode']=='CC'){?>
                            <option value="CC" data-charge="<?php echo $charge['our_charge'];?>" data-gst="<?php echo $charge['GST'];?>" data-type="<?php echo $charge['type'];?>">Credit Card (<?php echo $charge['our_charge'];?>% + <?php echo $charge['GST'];?>% GST) </option>
                            <?php }?>
                            <?php if($charge['mode']=='DC'){?>
                              <option value="DC"  data-charge="<?php echo $charge['our_charge'];?>" data-gst="<?php echo $charge['GST'];?>" data-type="<?php echo $charge['type'];?>">Debit Card(<?php echo $charge['our_charge'];?>%(below 2000) or 1%(above 2000) + <?php echo $charge['GST'];?>% GST)</option>
                            <?php }?>
                            <?php if($charge['mode']=='NB'){?>
                              <option value="NB"  data-charge="<?php echo $charge['our_charge'];?>" data-gst="<?php echo $charge['GST'];?>" data-type="<?php echo $charge['type'];?>">Net Banking(INR 15-20 +<?php echo $charge['GST'];?>% GST)</option>
                            <?php }?>
                            <?php if($charge['mode']=='UPI'){?>
                              <option value="UPI"  data-charge="<?php echo $charge['our_charge'];?>" data-gst="<?php echo $charge['GST'];?>" data-type="<?php echo $charge['type'];?>">UPI ( <?php echo $charge['our_charge'];?>% + <?php echo $charge['GST'];?>% GST)</option>
                            <?php }?>
                        ...
                        <div class="col-lg-12 no-pdd">
                          <div class="sn-field">
                           <label>	Transactions Charges  <strong style="color:#FF0000;">*</strong></label>
                           <input name="charges" id="charges" value="" class="form-control"  readonly/>
                           <!-- <i class="la la-lock"></i>--> </div>
                        </div>
                        <div class="col-lg-12 no-pdd">
                          <div class="sn-field">
                           <label> 	Total Payable Amount <strong style="color:#FF0000;">*</strong></label>
                           <input name="totalAmount" id="totalAmount"  class="form-control" value="" readonly/>
                           <!-- <i class="la la-lock"></i>--> </div>
                        </div>
                        <!-- <div class="col-lg-12 no-pdd">
                          <div class="sn-field">
                           <label>Address : </label>
                           <input name="address1" value="<?php echo (empty($posted['address1'])) ?  $rec[0]['house_number_id']. " Nirvana Country" : $posted['address1']; ?>" readonly />
                        <i class="la la-lock"></i> </div>
                        </div>
                       
                        <div class="col-lg-6 no-pdd">
                          <div class="sn-field">
                           <input name="city"  placeholder="City" value="<?php //echo (empty($posted['city'])) ? '' : $posted['city']; ?>" />
                           <i class="la la-user"></i></div>
                        </div>
                        <div class="col-lg-6 no-pdd">
                          <div class="sn-field">
                           <input name="state" placeholder="State" value="<?php //echo (empty($posted['state'])) ? '' : $posted['state']; ?>" />
                          <i class="la la-building-o"></i></div>
                        </div>
                        
                        <div class="col-lg-6 no-pdd">
                          <div class="sn-field">
                           <input name="country" placeholder="Country" value="<?php //echo (empty($posted['country'])) ? '' : $posted['country']; ?>" />
                           <i class="la la-user"></i> </div>
                        </div>
                        <div class="col-lg-6 no-pdd">
                          <div class="sn-field">
                           <input name="zipcode" placeholder="Zipcode" value="<?php //echo (empty($posted['zipcode'])) ? '' : $posted['zipcode']; ?>" />
                          <i class="la la-building-o"></i> </div>
                        </div> -->
                        ...
                        <div class="col-lg-12 no-pdd form-group">
                           <div class="checky-sec st2">
                            <div class="fgt-sec ">
                              ...
                              <small style="font-size: 16px;width: 96%;line-height: 25px;font-weight: 500;">Yes, I understand and agree to the <a href="<?php echo SITE_URL;?>tnc_events.html" data-target="#myModal" role="button" data-toggle="modal" style="color: #37a000;">Events Terms and Conditions </a>.</small> </div>
                            <!--fgt-sec end-->
                          </div>
                        </div>
                       
                        <div class="col-lg-3 no-pdd form-group">
                        <?php if(!$hash) { ?>
    ...
    if(mode=="CC" && typecal=='PERCENTAGE'){
     charges = (charge/100)*parseFloat(amount) + (charge/100)*(gst/100)*parseFloat(amount);
      ...
      if(amount<=2000){
        ...
        charges = (.01)*parseFloat(amount)+(0.18)*(.01)*parseFloat(amount);    
       }
