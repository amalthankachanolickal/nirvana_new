...
function sendMailAdmin($details, $url) {
     ...
     $to = 'kushalbhasin@gmail.com';

	 /*$to = 'iambommanakavya@gmail.com';*/
//  $to = 'iambommanakavya@gmail.com,techlead@myrwa.online';
  $subject = 'New Application Received for Plasma Donor Registration';
 ...
 if (isset($details['user_email']) && $details['user_email'] != '') {
  ...
  Kindly review the same by <a href="'.SITE_URL.'Plasma_Form.php?fid='.$url.'">clicking here</a> 
  
 
  <p style="margin:10px 0px;padding:0px;color:rgb(32,32,32);font-family:Helvetica;font-size:16px;line-height:24px;text-align:left">Warm Regards</p>
...
//   }
  ...
  <p style="margin:10px 0px;padding:0px;color:rgb(32,32,32);font-family:Helvetica;font-size:16px;line-height:24px;text-align:left">The office has received your information as below : </p>';
  ...
  <thead>
  <tr>
  <th style="border:1px solid black">Name of the Person(s)</th>
  <th style="border:1px solid black">Blood Group</th>
  <th style="border:1px solid black"># of Days since COVID Tested</th>
  <th style="border:1px solid black">Willing to donate Plasma</th>
  </tr>
  </thead>
  <tbody>
  <tr>
  <td style="border:1px solid black">'.$details['title_first'].' '.ucfirst($details['fname_first']).' '.ucfirst($details['mname_first']).' '.ucfirst($details['lname_first']).'</td>
  <td style="border:1px solid black">'.$details['first_blood_group'].'</td>
  <td style="border:1px solid black">'.$covid_positive_days.'</td>
  <td style="border:1px solid black">'.$plasma_donar.'</td>
  </tr>';
  if($details["fname_second"] <> NULL || $details["fname_second"] <> ""){
      $message .= '<tr>
  <td style="border:1px solid black">'.$details['title_second'].' '.ucfirst($details['fname_second']).' '.ucfirst($details['mname_second']).' '.ucfirst($details['lname_second']).'</td>
  <td style="border:1px solid black">'.$details['second_blood_group'].'</td>
  <td style="border:1px solid black">'.$covid_positive_days.'</td>
  <td style="border:1px solid black">'.$plasma_donar.'</td>
  </tr>';
  ...
  if($details["fname_third"] <> NULL || $details["fname_third"] <> ""){
   $message .= '<tr>
  <td style="border:1px solid black">'.$details['title_third'].' '.ucfirst($details['fname_third']).' '.ucfirst($details['mname_third']).' '.ucfirst($details['lname_third']).'</td>
  <td style="border:1px solid black">'.$details['third_blood_group'].'</td>
  <td style="border:1px solid black">'.$covid_positive_days.'</td>
  <td style="border:1px solid black">'.$plasma_donar.'</td>
  </tr>';
  ...
  <p style="margin:10px 0px;padding:0px;color:rgb(32,32,32);font-family:Helvetica;font-size:16px;line-height:24px;text-align:left">Address : '.getBlock($details['block_id']).'-'.$details['house_number'].' '.getFloor($details['floor_no']).', Nirvana Country, Gurgaon - 122018</p>
  <p style="margin:10px 0px;padding:0px;color:rgb(32,32,32);font-family:Helvetica;font-size:16px;line-height:24px;text-align:left">Mobile Number : '.$details['isd_code'].'-'.$details['phone_number'].'</p>
  <p style="margin:10px 0px;padding:0px;color:rgb(32,32,32);font-family:Helvetica;font-size:16px;line-height:24px;text-align:left">Email Address : '.$details['user_email'].'</p>
...
$user_id_data = (isset($_SESSION['UR_LOGINID'])) ? $_SESSION['UR_LOGINID'] : NULL;
    ...
    $positive_covid_test_data = (isset($_POST['date_of_positive_covid_test'])) ? $_POST['date_of_positive_covid_test'] : NULL;
    $negative_covid_test_data = (isset($_POST['date_of_negative_covid_test'])) ? $_POST['date_of_negative_covid_test'] : NULL;
...
//  print_r($ModelCall);
 // exit;
  ...
  $block = getBlock($insert_data1['block_id']);

  $house_number = $insert_data1['house_number'];

  if (strlen($insert_data1['house_number']) == "1") {
    $house_number = '000'.$insert_data['house_number'];
  } else if (strlen($insert_data1['house_number']) == "2") {
    $house_number = '00'.$insert_data1['house_number'];
  } else if (strlen($insert_data1['house_number']) == "3") {
    $house_number = '0'.$insert_data1['house_number'];
  }
  ...
  $floor_no_in = getFloor($insert_data1['floor_no']);

  $itaq_data = array(
    "villa_number" => getBlock($insert_data1['block_id']) . ' ' . $insert_data1['house_number'] . ' ' . getFloor($insert_data1['floor_no']),
    "quar_start_date" => date("d M y", strtotime($insert_data1['app_date'])),
    "quar_end_date" => date("d M y", strtotime($insert_data1['end_date'])),
    "remarks" => (strlen($insert_data1['place']) > 0) ? 'Travelled from ' . $insert_data['place'] : ''
  );
 ...
 // sendMailAdmin($insert_data1, base64_encode($insert_id));
  unset($_SESSION['user_data']);
  header("Location: ../Plasma_Form.php?actionResult=sent");
}
?>