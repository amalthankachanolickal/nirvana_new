                 ...
                 <td><?= getBlock($getDocumentVal['block_applicants']) . '-' . $getDocumentVal['house_number_applicants'] . '-' . getFloor($getDocumentVal['floor_number_applicants']) ?></td>
                   <td><?= ucfirst($getDocumentVal['user_title_one']) . ' ' . ucfirst($getDocumentVal['first_name_one']) . ' ' . ucfirst($getDocumentVal['mid_name_one']) . ' ' . ucfirst($getDocumentVal['last_name_one']) ?></td>
                    <td><?= ($getDocumentVal['approved_status'] == "0") ? 'Pending Approval' : 'Approved' ?></td>
                      ...
                      <td class="text-right"><a data-toggle="modal" data-target="#add_approve_comment_<?= $getDocumentVal['id'] . '_' . $frm ?>"  href="#" class="btn btn-success">Approve</a>
                      </td>
                      <td class="text-right"><a data-toggle="modal" data-target="#add_decline_comment_<?= $getDocumentVal['id'] . '_' . $frm ?>" href="#" class="btn btn-danger">Decline</a>
                      </td>

  <?php include('add_approve_feedback.php'); ?>
  <?php include('add_decline_feedback.php'); ?>
                    <?php } else { ?>
	...
	foreach($getDocumentInfo_tenant as $getDocumentVal){ ?>             
                ...
                <td><?= ucfirst($getDocumentVal['form_name']) ?></td>
                 <td><?= getBlock($getDocumentVal['block_id']) . '-' . $getDocumentVal['house_number'] . '-' . getFloor($getDocumentVal['floor_number']) ?></td>
                  ...
                  <?php if($getDocumentVal['approved_status']== "0"){?>      
                      <td class="text-right"><a data-toggle="modal" data-target="#add_approve_comment_<?= $getDocumentVal['id'] . '_' . $frm ?>" class="btn btn-success">Approve</a>
                      </td>
                      <td class="text-right"><a data-toggle="modal" data-target="#add_decline_comment_<?= $getDocumentVal['id'] . '_' . $frm ?>" class="btn btn-danger">Decline</a>
                      </td>
