    ...
    <div class="content container-fluid">
      <div class="row">
       <?php if($_REQUEST['actionResult']!='') { include('messageDisplay.php'); }?>
        <div class="col-md-12">
        <h3 class="page-title">Send Custom Notifications to multiple users</h3>
          <form  action="<?php echo DOMAIN.AdminDirectory;?>controller/send-notification-actions.php" method="post" enctype="multipart/form-data"  class="form-horizontal"  style="border:1px solid black;padding: 5px 5px;padding-left:15px;" onSubmit="return ValidateSubmitForm();">
          <input type="hidden" name="ActionCall" value="sending_emails">
            <input type="hidden" name="eid" value="<?php echo $getAdminInfo[0]['id'];?>">
            
            <input type="hidden" name="selectedComType" id="selectedComType" >
            ...
            <input type="hidden" name="selectedMemberStatusLabels" id="selectedMemberStatusLabels" >
                ...
                <div class="form-group">
                    <label class="control-label  col-sm-3" for="com_type">Choose Communication Type:</label>
                    <div class="col-sm-9">
                    <label class="checkbox-inline"><input type="checkbox" value="email" name="com_type" ><i class="fa fa-envelope-o" aria-hidden="true"></i> Mails</label>
                    <label class="checkbox-inline"><input type="checkbox" value="sms" name="com_type" ><i class="fa fa-commenting-o" aria-hidden="true"></i> SMS</label>
                    <label class="checkbox-inline"><input type="checkbox" value="whatsapp" name="com_type" disabled  ><i class="fa fa-whatsapp" aria-hidden="true"></i> WhatsApp</label>
                    </div>
                </div>
                <hr>
                ...
                <!--Communication Content Selection -->
                
                <ul class="nav nav-tabs">
                  <li class="active"><a data-toggle="tab" href="#email"><i class="fa fa-envelope-o" aria-hidden="true"></i> Email</a></li>
                  <li><a data-toggle="tab" href="#sms"><i class="fa fa-commenting-o" aria-hidden="true"></i> SMS</a></li>
                  <li><a data-toggle=""  href="#whatsapp"><i class="fa fa-whatsapp" aria-hidden="true"></i> WhatsApp</a></li>
                </ul>
                
                <div class="tab-content">
                  <div id="email" class="tab-pane fade in active">
                    <?php include("template_email.php");?>
                  
                  </div>
                  <div id="sms" class="tab-pane fade">
                    <?php include("template_sms.php");?>
                 </div>
            ...
            <div class="row">
              <div class="col-sm-12 text-center m-t-20">
                ...
                <button style="margin-right:5px;" type="submit" name="submitAction" value="downloadExcel" class="btn btn-warning"><i class="fa fa-download"></i> Download Receiver List</buton>
                <button type="submit" name="submitAction" value="sendNotifi" class="btn btn-primary">Send Notification</button>
              </div>
            </div>
          </form>
...
function ValidateSubmitForm(){
    ...
    var selectedComType = new Array();
     $('input[name="com_type"]:checked').each(function() {
        selectedComType.push(this.value);
    });
    
    if(selectedComType.length==0){
        alert("Please select communication type.");
        return false;
     }
    
    if(selectedComType.includes("email")){
        if($("#subject").val()==""){
            alert("Email Subject can not be empty in Email Tab.");
            return false;
        }
        
        if (tinymce.EditorManager.get('content').getContent() === '') {
            alert('Email Content can not be empty in Email Tab.');
            return false;
        }
    }
    
    if(selectedComType.includes("sms")){
        if($("#template").val()==""){
            alert("Please Select SMS content template in SMS Tab.");
            return false;
        }
    }
    ...
    $("#selectedComType").val(JSON.stringify(selectedComType));
    ...
    // alert("selectedComType : "+JSON.stringify(selectedComType));
    // alert("selectedblockids :"+JSON.stringify(selectedblockids));
    // alert("selectedMemberTypes :"+JSON.stringify(selectedMemberTypes));
    // alert("selectedMemberResiding :"+JSON.stringify(selectedMemberResiding));
    // alert("selectedMemberStatus :"+JSON.stringify(selectedMemberStatus));
