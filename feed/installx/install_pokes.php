...
if(isset($_POST['submit'])) {
	...
	$p_version = "1.1.4";
	$p_type = "Pokes";
	$p_desc = "This plugin gives to your users the ability send a poke a friends o other users of your network.";
	$p_icon = "poke";
	...
	if(!$resource->num_rows){ 
		...
		VALUES ( '$p_name',  '$p_version',  '$p_type',  '$p_desc',  '$p_icon', '10', '1')"; 
		$sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");
	} else {// UPDATE plugins
	...
	/*create tab pokes*/ 
	$sql = "SHOW TABLES FROM `".$sql_db_name."` LIKE 'pokes'";
	$resource = $sqlConnect->query($sql) or die($sqlConnect->error." <b>SQL was: </b>$sql");	
	if( !$resource->num_rows ){  
		$sql = "
	...
	//install plugins lang pokes
	foreach($wo['plugin_poke'] as $k => $v){
		$langz = "";
		$new_k = 'plugin_poke_'.str_replace("-",'_',$k);
		$sql = "SELECT * FROM `Wo_Langs` WHERE lang_key='{$new_k}' LIMIT 1";
		$resource = $sqlConnect->query($sql);
		if(!$resource->num_rows ){
			$sqlConnect->query("INSERT INTO `Wo_Langs` (`lang_key`) VALUES ('{$new_k}')");
...
<html>
    <head>
        ...
        <title>Plugins for Wowonder v1.5 &rsaquo; Installer</title>      
			...
			<div class="row">
				<div class="col-sm-12">
					...
					<div class="panel panel-default">               
						<div class="panel-heading"> Welcome to <strong>Poke Plugin</strong> installation!</div>
						<div class="panel-body pg_1x">	
							...
							<form autocomplete="off" action="install_pokes.php" method="post">                             
								<?php if($ready == 0){
