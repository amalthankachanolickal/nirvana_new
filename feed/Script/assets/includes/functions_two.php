...
function Wo_GetAdvertiseBannersPage() {
    global $sqlConnect;
	echo  "SELECT * FROM tbl_ads_management_directory WHERE `status` = '1'  ORDER BY RAND()";
    $query_one      = mysqli_query($sqlConnect, "SELECT * FROM tbl_ads_management_directory WHERE `status` = '1'  ORDER BY RAND()");
   $fetched_data = mysqli_fetch_assoc($query_one);
    return $fetched_data;
}
