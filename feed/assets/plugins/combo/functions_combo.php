...
function getReactButtonTemplate($r = '', $user_id, $post_id){ 
	...
	if ($reaction = isReacted($r, $user_id, $post_id)){ 
		...
		return '<span class="text-link story-like-btn opt" onclick="set_reaction($(this), \'like\', '.$post_id.');" title="'.$wo['lang']['like'].'"><i class="fa fa-thumbs-o-up progress-icon" data-icon="thumbs-up"></i> '.$wo['lang']['like'].'</span>';
	}
}
