  ...
  */
    ...
    /*Search Suggest friend poke*/
      ...
      var _this = $(this);
        ...
        if(raw_query !== null && raw_query.length > 0) {
            ...
            $.post( site_ajax+"/?f=autocomplete&s=users&type=poke&only=pokes", {'query': query}, function(response) {
    ...
    $('body').on('click', function(e) {
     ...
     function poke_list_send(self){
          ...
          $.ajax({ type:'post', url: site_ajax, data: {'f':'pokes', 's':'send', 'owner_id':id, 'only':'pokes' }, cache:false, success: function(result) { 
              send_poke_active = 1;
			  if(result.result == 1){ 
    ...
    function delete_poke(self){ 
         ...
         $.ajax({ type: 'post', url: site_ajax, data: {'f':'pokes', 's':'delete', 'owner_id':id, 'only':'pokes' }, cache : false, success: function(result) { 
	...
	//suggest click
	$('body').on('click', '.js_poke_add', function(e){ e.preventDefault(); var self = $(this); poke_list_send(self); });
	
	//button delete poke
	$('body').on('click', '.delete_poke', function(e){ e.preventDefault(); var self = $(this); delete_poke(self); });
	
	//button send poke
	$('body').on('click', '.btn_poke', function(e){ e.preventDefault(); var self = $(this); poke(self); });
	
	//button send poke in profile
	$('body').on('click', '.js_poke-user', function(e){ e.preventDefault(); var id = $(this).attr('data-uid'); profile_poke(id); });