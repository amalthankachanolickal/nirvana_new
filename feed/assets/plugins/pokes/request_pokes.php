	...
	if($s == 'send'){		 
		...
		if($owner_id != $wo['user']['user_id']){
			...
			if ( $toque_exist == 0 ){
				...
				$noty_data_array = array('recipient_id' => $to_user_id, 'type' => 'poke', 'url' => 'index.php?link1=pokes', 'email_subject' =>$subject, 'email_text' => $body);
				$test = Register_Notification($noty_data_array);		
